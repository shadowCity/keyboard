define(function () {
  var tpl = requirejs('text!sys_components/cashboard/cashboard.html')
  return {
    name: 'cashboard',
    template: tpl,
    props: {
    ...
    data: function(){
      return {
        ...
        numArray4: [{ value: '.', type: 'key' }, { value: '0', type: 'key' }, { value: '00', type: 'key' }],
   ...
   methods: {
    ...
    keyclick(value,type){
      switch(type){
          case 'key':
            var preval = this.$store.state.keyboardValue_cash.split('') //获取之前的值，炸成数组
            preval.splice(this.$root.pointer,0,value) //使用数组splice方法进行插入
            this.$store.state.keyboardValue_cash = preval.join('')  //在合成字符串赋给状态
            this.$root.pointer += value.length //光标位置+1
            break
          case 'hide':
          case 'done':
            this.$store.commit("hideKeyboard_cash")
            break
          case 'del':
            this.keyValue = this.$store.state.keyboardValue_cash
            this.$store.state.keyboardValue_cash = this.keyValue.substr(0, this.$root.pointer - 1) + this.keyValue.substr(this.$root.pointer)
